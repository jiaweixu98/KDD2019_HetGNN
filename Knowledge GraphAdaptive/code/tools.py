...
# 这里调用了，a_content_agg，前面已经处理好了
		if node_type == 1:
			...
			neigh_agg = self.p_content_agg(id_batch).view(batch_s, 5, embed_d)
			neigh_agg = torch.transpose(neigh_agg, 0, 1)
			all_state, last_state  = self.p_neigh_rnn(neigh_agg)
		else:
			neigh_agg = self.v_content_agg(id_batch).view(batch_s, 5, embed_d)
			neigh_agg = torch.transpose(neigh_agg, 0, 1)
	...
	def node_het_agg(self, id_batch, node_type): #heterogeneous neighbor aggregation
		a_neigh_batch = [[0] * 5] * len(id_batch)
		p_neigh_batch = [[0] * 5] * len(id_batch)
		v_neigh_batch = [[0] * 5] * len(id_batch)
