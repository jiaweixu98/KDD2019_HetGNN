...
# text embedding 暂不考虑
			...
			# v_text_embed = np.zeros((self.args.A_n, self.args.in_f_d * 3))
...
# 这块要随机游走先得到，这里待会儿必须先跑随机游走，需要het_neigh_train.txt；这里的采样数量（10，10，3）和后面的a_L, v_L以及p_L
			...
			top_k = [5, 5, 5] #fix each neighor type size
			for i in range(self.args.A_n):
				for j in range(3):
		...
		for i in range(3):
			for j in tqdm(range(node_n[i])):
				...
				if len(neigh_temp):
					...
					# 加一些限制条件，假如返回最初的起点超过10次，说明大概率找不到100个节点了，就continue
					back_count = 0
					neigh_L = 0
					a_L = 0
					p_L = 0
					v_L = 0
					while neigh_L < 50: #maximum neighbor size = 100; 所以原文是100个邻居，45个paper，45个作者，10个venue.我们改成34，33，33
						rand_p = random.random() #return p； 返回的概率
						if rand_p > 0.5:
							if curNode[0] == "a":
								curNode = random.choice(self.a_p_list_train[int(curNode[1:])])
								if p_L < 18: #size constraint (make sure each type of neighobr is sampled)
									neigh_train.append(curNode)
									neigh_L += 1
									p_L += 1
							elif curNode[0] == "p":
								curNode = random.choice(self.p_neigh_list_train[int(curNode[1:])])
								# if curNode != ('a' + str(j)) and curNode[0] == 'a' and a_L < 34:
								# 必须给p找到a的机会，因为有的p只有一个对应的a
								if curNode[0] == 'a' and a_L < 18:
									neigh_train.append(curNode)
									neigh_L += 1
									a_L += 1
								elif curNode[0] == 'v':
									# 这里也是问题，为啥v_L就要小？这里选择的v比较少，因为原数据集中，认为v太泛化，作用不大
									if v_L < 17:
										neigh_train.append(curNode)
										neigh_L += 1
										v_L += 1
